...
const PUBLIC_EXACT = new Set([
  '/player', '/player.html', '/player.js', '/player-layout.js',
...
function createAuth(options) {
  ...
  // Sessions must survive a service restart, so the signing key is derived from the
  // password, plus SESSION_SECRET when one is set. Changing either logs everyone out,
  // which is what you want after a password has leaked.
  const secret = options.secret
    ? sha256('signage-session:' + options.secret + ':' + password)
