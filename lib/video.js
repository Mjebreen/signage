...
function createVideoTurner(options) {
  ...
  function pump() {
    ...
    const finish = ok => {
      ...
      if (!ok && !job.retried && !fs.existsSync(outDir)) {
        // The folder was deleted under a running transcode (the README allows that): not
        // the video's fault, so make it again, once, instead of marking it failed.
        queue.unshift({ file: job.file, deg: job.deg, retried: true });
        pump();
        return;
      }
      if (!ok) {
